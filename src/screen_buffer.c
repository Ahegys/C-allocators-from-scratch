...
// }
void draw_window_border(wchar_t* buffer) {
    write_to_screen_buffer(buffer, 0, 0, L'Q');
    write_to_screen_buffer(buffer, WIDTH - 1, 0, L'Q');
    write_to_screen_buffer(buffer, 0, HEIGHT - 1, L'Q');
    write_to_screen_buffer(buffer, WIDTH - 1, HEIGHT - 1, L'Q');
    for (int x = 1; x < WIDTH - 1; x++) {
        write_to_screen_buffer(buffer, x, 0, L'Q');
        write_to_screen_buffer(buffer, x, HEIGHT - 1, L'Q');
    }
    for (int y = 1; y < HEIGHT - 1; y++) {
        write_to_screen_buffer(buffer, 0, y, L'Q');
        write_to_screen_buffer(buffer, WIDTH - 1, y, L'Q');
    }
}
...
void draw_table_widget(wchar_t* buffer, int start_x, int start_y, int rows, int cols, const wchar_t* content[rows][cols]) {
    ...
    write_to_screen_buffer(buffer, start_x, start_y, L'Q');
    for (int c = 0; c < cols; c++) {
        for (int i = 0; i < col_width; i++) {
            write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + i, start_y, L'Q');
        }
        if (c < cols - 1) {
            write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + col_width, start_y, L'Q');
        }
    }
    write_to_screen_buffer(buffer, start_x + table_width - 1, start_y, L'Q');

    for (int r = 0; r < rows; r++) {
        if (r > 0) {
            write_to_screen_buffer(buffer, start_x, start_y + r * (row_height + 1), L'Q');
            for (int c = 0; c < cols; c++) {
                for (int i = 0; i < col_width; i++) {
                    write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + i, start_y + r * (row_height + 1), L'Q');
                }
                if (c < cols - 1) {
                    write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + col_width, start_y + r * (row_height + 1), L'Q');
                }
            }
            write_to_screen_buffer(buffer, start_x + table_width - 1, start_y + r * (row_height + 1), L'Q');
        }
        for (int c = 0; c < cols; c++) {
            write_to_screen_buffer(buffer, start_x + c * (col_width + 1), start_y + r * (row_height + 1) + 1, L'Q');
            if (content[r][c]) {
                int len = wcslen(content[r][c]);
                int padding = (col_width - len) / 2;
                int text_x = start_x + 1 + c * (col_width + 1) + padding;
        ...
        write_to_screen_buffer(buffer, start_x + table_width - 1, start_y + r * (row_height + 1) + 1, L'Q');
    }

    write_to_screen_buffer(buffer, start_x, start_y + table_height - 1, L'└');
    for (int c = 0; c < cols; c++) {
        for (int i = 0; i < col_width; i++) {
            write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + i, start_y + table_height - 1, L'Q');
        }
        if (c < cols - 1) {
            write_to_screen_buffer(buffer, start_x + 1 + c * (col_width + 1) + col_width, start_y + table_height - 1, L'Q');
        }
    }
    write_to_screen_buffer(buffer, start_x + table_width - 1, start_y + table_height - 1, L'Q');
}